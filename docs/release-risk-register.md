...
| Production environment drift | `.env.example`, `lib/server/session-cookie.js`, `api/encore-proxy.js`, `server.ts`, `.github/workflows/staging-smoke.yml` | `ENCORE_API_URL` fails closed in production-like environments; staging backend is blocked unless explicitly allowed; staging smoke validates required secrets. | Production-sensitive releases must verify `ENCORE_API_URL`, `YOCO_PAYMENT_MODE`, `YOCO_SECRET_KEY`, `YOCO_TEST_SECRET_KEY`, `YOCO_WEBHOOK_SECRET`, `RESEND_API_KEY`, `AUTH_EMAIL_FROM`, `GOOGLE_OAUTH_CLIENT_ID`, `GEMINI_API_KEY`, `DEEPSEEK_API_KEY`, Vercel host, and Encore environment alignment without exposing secret values. |
